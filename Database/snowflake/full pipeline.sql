CREATE OR REPLACE STAGE my_azure_stage
  ...
  use TPCH.EXTERNAL_TABLE ;


CREATE OR REPLACE EXTERNAL TABLE part
  USING TEMPLATE (
  ...
  CREATE OR REPLACE EXTERNAL TABLE partsupp
  USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
      FROM TABLE(
        INFER_SCHEMA(
  ...
  USING TEMPLATE (
    ...
    CREATE OR REPLACE EXTERNAL TABLE lineitem
  USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
      FROM TABLE(
        INFER_SCHEMA(
  ...
  USING TEMPLATE (
    ...
    AUTO_REFRESH=false;
     ...
     CREATE OR REPLACE EXTERNAL TABLE region
  USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
      FROM TABLE(
        INFER_SCHEMA(
 ...
 CREATE OR REPLACE EXTERNAL TABLE nation
  USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
      FROM TABLE(
        INFER_SCHEMA(
...
CREATE OR REPLACE EXTERNAL TABLE supplier
  USING TEMPLATE (
    SELECT ARRAY_AGG(OBJECT_CONSTRUCT(*))
      FROM TABLE(
        INFER_SCHEMA(
